...
def freyface():
    raw_faces = pickle.load(open(curr_path + "/data/freyfaces.pkl", "rb"))
...
def load_stopwords():
    stop_words = []
    file = open("/home/zpl/neural-topic-model-master/data/stopwords.json", "r")
    f = file.readlines()
    file.close()
    for line in f:
...
def load(file_path):
    dic = {}   # 单词的词频
    i2w = {}   # 索引-字符表{index： char}
    w2i = {}   # 字符-索引表{char： index}
    docs = {}   # document-字袋{doc_id: words},一个document一行
...
def batched_idx(lst, batch_size = 1):   # 每个batch_id对应20条document
    np.random.shuffle(lst)
    data_xy = {}
    batch_x = []
    batch_id = 0
...
def batched_news(x_idx, data):
    ...
    X = np.zeros((len(x_idx), len(dic)), dtype = theano.config.floatX)    ## X.shape：batch_size * vocab_size
